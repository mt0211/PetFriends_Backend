...
public class RabbitMQService : IMessageBus
{
    ...
    private const string ClinicServiceNotificationExchangeName = "clinic_service_notification_events";
    public RabbitMQService()
    {
        ...
        _channel.ExchangeDeclare(ClinicServiceNotificationExchangeName, ExchangeType.Fanout);
    }
    ...
    public void PublicClinicServiceNotification(string type, Guid clinicServiceId)
    {
        var message = JsonSerializer.Serialize(new
        {
            Type = type,
            ClinicalServiceId = clinicServiceId,
            Timestamp = DateTime.UtcNow
        });
        var body = Encoding.UTF8.GetBytes(message);
        _channel.BasicPublish(
           exchange: ClinicServiceNotificationExchangeName,
           routingKey: "",
           basicProperties: null,
           body: body);
    }
